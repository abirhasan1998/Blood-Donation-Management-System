...
<form action="" method="POST">
  ...
  <select name="E_ID">
	...
	?>
	<option value="<?php echo $row1['Patient_Id']; ?>">
		<?php
...
<div class="employee">
  ...
  <?php
	  ...
	  $SQL = "SELECT * FROM tb_patient WHERE '".$a."' IN (Patient_Id);";
		  $result=mysqli_query($connection,$SQL);
		  if(!$result || mysqli_num_rows($result) > 0)
			{
			  echo "<table><tr><th>Patient_Name</th><th>Patient_Id</th><th>Patient_Blood_Group</th><th>Address</th><th>Contact_No</th></tr>";
			  while($row=mysqli_fetch_array($result))
				{  
				 
                 echo "<tr><td>".$row['Patient_Name']."</td><td>".$row['Patient_Id']."</td><td>".$row['Patient_Blood_Group']."</td><td>".$row['Address']."</td><td>".$row['Contact_No']."</td></tr>";
				}
			  echo "</table>";
			}
